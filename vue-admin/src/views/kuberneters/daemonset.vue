...
import { daemonsets } from "@/api/k8s/daemonset";
...
export default {
  ...
  methods: {
    ...
    fetchdaemonset(namespace) {
      this.activeNamespace = namespace;
      daemonsets(this.activeCluster, this.activeNamespace).then((response) => {
        this.daemonsetList = response.data.items;
      });
    },
  },
};
